...
(function() {
    ...
    var H = [0, 1, 2, 4, 8, 16, 32, 64, 128, 27, 54]
      , d = d.AES = p.extend({
        lh1x: function() {
            for (var a = this.J8B, c = a.words, d = a.sigBytes / 4, a = 4 * ((this.ZK7D = d + 6) + 1), e = this.bqB0x = [], j = 0; j < a; j++)
                if (j < d)
                    e[j] = c[j];
                else {
                    var k = e[j - 1];
        ...
        EJ9A: function(a, b, c, d, e, j, l, f) {
            for (var m = this.ZK7D, g = a[b] ^ c[0], h = a[b + 1] ^ c[1], k = a[b + 2] ^ c[2], n = a[b + 3] ^ c[3], p = 4, r = 1; r < m; r++)
...
function start(id) {
    ...
    var bVV8N = asrsea(JSON.stringify(d), e, f, g);
    var data = {
